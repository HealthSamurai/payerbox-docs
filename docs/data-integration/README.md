...
| Keys (USCDI feed) | Each table names the column, or columns, that key its rows. Keep those values stable: the resource id derives from them, so a change creates a second resource instead of updating one. `is_deleted` set to `true` retracts the row. |
