...
- `last_updated` is mandatory on every claim, but it does not become `meta.lastUpdated`: CARIN requires that element, and FHIR reserves it for the server that stores the resource, so Payerbox stamps it with the time the claim was ingested rather than taking your value. Send `last_updated` anyway — it is your own record of when the claim changed, independent of when we received it.
