...
| `formulary_aliases` | If renamed | earlier names, `;`-separated, up to three, so old searches still find it | |
...
| `is_deleted` | If retracting | `true` sets the published formulary's `status` to `retired`; its identifier and contact rows go with it | `true` |
