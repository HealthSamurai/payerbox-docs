...
| `phone` | If available | 10 digits; several `;`-separated | |
| `fax` | If available | 10 digits; several `;`-separated | |
| `email` | If available | email addresses, `;`-separated | |
| `url` | If available | web addresses, `;`-separated | `https://example.org/formularies/form-2027-a.pdf` |
...
- A row needs a `contact_name` or at least one telecom value; otherwise it is rejected.
