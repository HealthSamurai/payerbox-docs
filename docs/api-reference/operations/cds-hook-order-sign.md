...
Coverage determinations are delivered as `systemActions[]` — one `update` action per evaluated order, carrying the draft order with the `ext-coverage-information` extension appended. Cards are informational only (summary, indicator, detail, links, source). See [CRD / System actions](../../prior-auth/crd.md#system-actions) for the extension's sub-extensions and the DTR launch mechanism.
