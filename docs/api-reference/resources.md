...
Each resource above is constrained by the relevant IG profile when interacting with a specific surface. For example, `Patient` on Patient Access conforms to `us-core-patient`; on Plan Net it conforms to `plannet-Practitioner` for the providers in the directory.
