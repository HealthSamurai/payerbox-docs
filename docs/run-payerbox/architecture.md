...
|---|---|---|
| **Aidbox** | FHIR engine | FHIR R4 storage, search, validation, terminology, SMART App Launch and Backend Services auth, BALP audit log, CapabilityStatement, bulk export, Multibox multi-tenancy |
| **Interop** | CMS APIs application layer | Patient Access, Provider Access, Payer-to-Payer, Provider Directory endpoints. Implements `$provider-member-match`, `$bulk-member-match`, `$davinci-data-export` and the consent and attribution semantics those operations require |
| **Prior Auth** | ePA application layer | CRD CDS Hooks endpoints, DTR `$questionnaire-package`, PAS `$submit` / `$inquire` / `$submit-attachment`, X12 278 mapping |
| **Portals** | Web UIs | Admin Portal, Developer Portal, Smart App Gallery. Single Docker image with Nginx domain-based routing |
...
sequenceDiagram
  ...
  interop->>client: response (Bundle, ClaimResponse, AsyncJob)
```
...
| Provider Directory | Anyone (public) | Caller → Interop public Plan-Net endpoints → Aidbox storage |
| CRD | EHR (CDS Hooks) | EHR → Prior Auth CDS Hooks endpoint → Aidbox (for member context) → external decision service → response cards |
| DTR | EHR (SMART app) | EHR → Prior Auth `$questionnaire-package` → Aidbox storage (Questionnaire, Library); CQL runs in the DTR SMART app, not on Aidbox |
| PAS | EHR / UM vendor (Backend Services) | Client → Aidbox /token → Prior Auth `$submit` → ClaimResponse stored in Aidbox; X12 278 mapping happens inside Prior Auth |
...
See [Connect Systems](README.md) for adapter details.

> TODO(engineering): document supported ETL formats and the X12 protocol options (REST gateway, MLLP, sftp).
...
## Default ports in the development bundle

| Service | Port | URL |
|---|---|---|
| Postgres | 5432 (internal) / 5444 (host) | — |
| Aidbox FHIR | 8090 | `http://dev.localhost:8090` |
| Aidbox Admin (multi-tenant control) | 8080 | `http://admin.localhost:8080` |
| Portals (Admin + Developer + App Gallery) | configurable | Domain-based routing |
| Interop | configurable | TODO(engineering) |
| Prior Auth | configurable | TODO(engineering) |

> TODO(engineering): finalize default ports for Interop and Prior Auth in the production bundle.
